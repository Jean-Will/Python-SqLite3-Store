...
def menu():
    while True:
        ...
        if escolha == '1':
            ...
            categorias = { "1": "Livraria", "2": "Roupas", "3": "Acessórios" }
            categoria_nome = categorias.get(cat_opcao)
            

            if categoria_nome:
                insertCategoria(categoria_nome)  # Verifica ou insere a categoria
...
def insertCategoria(categoria):
    try:
        with sqlite3.connect('agapeshop.db') as conn:
            ...
            cursor.execute(''' INSERT OR IGNORE INTO categoria (nome_categoria) VALUES (?)''',(categoria,))
            conn.commit() 

            print("Categoria verificada ou inserida com sucesso \n")
